...
parser = argparse.ArgumentParser(description='Process a root file and create a h5 file containing the useful features for the NN')
...
myInput = rt.TFile.Open("/afs/cern.ch/work/f/fbury/private/"+str(args.input_file))

tree = myInput.Get("pixelHits/treeAll")

# Input Data #
nentries = tree.Draw("1","hit1_layer > 0 && pt >= 20","GOFF")
if args.x>nentries :
...
for i in xrange(args.x, maxent):
	...
	continue
    if myEvent[36]<20:   # if pt small => dump (probably Background)
	continue
...
Out = h5py.File('/afs/cern.ch/work/f/fbury/h5_files/'+args.output_file, 'w')
